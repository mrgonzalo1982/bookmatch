...
function AdMínView({ onBack }) {
  ...
  const [bookForm, setBookForm] = useState({ title: '', author: '', description: '', genre: '', MínNivel: 5, maxNivel: 12, image: '' });
    ...
    try {
      if (activeTab === 'books') {
        ...
        const list = snap.docs
          ...
          .filter(u => u.role === 'teacher' || u.role === 'adMín');
        setTeachers(list);
          ...
          studentsWithLikes.forEach((s1, i) => {
            studentsWithLikes.slice(i + 1).forEach(s2 => {
              ...
              if (score > 10) { // Only count meaningful connections
                ...
                twins.push({ s1: name1.split(' ')[0] + ' ' + (name1.split(' ')[1] || ''), s2: name2.split(' ')[0] + ' ' + (name2.split(' ')[1] || ''), score, comMín: s1.likes.filter(l => s2.likes.some(l2 => l2.id === l.id)).length });
              }
            });
          });
    ...
    try {
      ...
      if (data.items && data.items.length > 0) {
        ...
        setBookForm({
          ...
          MínNivel: 5,
          maxNivel: 12,
          image: book.imageLinks?.thuMínail || 'https://images.unsplash.com/photo-1543002588-bfa74002ed7e?auto=format&fit=crop&q=80&w=400'
        });
      } else {
  ...
  const handleSaveBook = async (e) => {
    ...
    try {
      ...
      setEditingBook(null);
      setBookForm({ title: '', author: '', description: '', genre: '', MínNivel: 5, maxNivel: 12, image: '' });
      fetchData();
      alert('Libro guardado exitosaMínte');
    } catch (err) { alert('Error al guardar libro'); }
  };

  const deleteBook = async (id) => {
    if (!window.confirm("Â¿EliMínar este libro definitivaMínte?")) return;
    try {
  ...
  const handleSaveTeacher = async (e) => {
    ...
    const data = { 
      role: 'teacher', 
      profile: { name: teacherForMíname, dept: teacherForm.dept, emoji: 'ðŸ“š', genres: [] },
      likes: []
    };
    ...
    try {
      ...
      fetchData();
      alert('Profesor guardado exitosaMínte');
    } catch (err) { alert('Error al guardar profesor'); }
  };

  const deleteTeacher = async (rut) => {
    if (!window.confirm("Â¿EliMínar acceso de este profesor?")) return;
    try {
  ...
  const deleteStudentData = async (rut) => {
    if (!window.confirm("Â¿Borrar definitivaMínte los datos de este alumno?")) return;
    const cleanRut = rut.replace(/[^0-9kK]/gi, '').toLowerCase();
    try {
      await deleteDoc(doc(db, 'users', cleanRut));
      alert("Datos eliMínados de Firestore");
    } catch (e) { alert("Error al borrar"); }
  };
  ...
  const exportToExcel = async () => {
    try {
      const snap = await getDocs(collection(db, 'users'));
      let csvContent = "RUT Alumno\tNombre\tCurso\tID Libro\tTítulo Libro\tAutor\n";
      ...
      const link = docuMínt.createEleMínt("a");
  ...
  const handlePurgeFirestore = async () => {
    ...
    try {
      ...
      alert(`âœ… Â¡Listo! Se limpiaron ${catSnap.size} libros. Profesores falsos eliMínados.`);
      fetchData();
  ...
  const courses = useMemo(() => Array.froMínew Set(STUDENTS.map(s => s.curso))).sort(), []);
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#F7F7F9] overflow-hidden">
      ...
      <div className="p-4 bg-[#F7F7F9] flex gap-2">
        ...
        {activeTab === 'books' && (
          <button onClick={() => { setEditingBook(null); setBookForm({ title: '', author: '', description: '', genre: '', MínNivel: 5, maxNivel: 12, image: '' }); setShowBookModal(true); }} className="bg-[#A80A0A] text-white px-4 rounded-xl shadow-lg hover:scale-105 active:scale-95 transition-all text-[10px] font-black uppercase tracking-widest">AÑADIR</button>
        )}
        {activeTab === 'teachers' && (
              ...
              <div key={teacher.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[8px] font-black bg-blue-50 text-blue-600 px-2 py-0.5 rounded uppercase tracking-widest mt-1 inline-block">{teacher.profile?.dept || 'Sin DepartaMínto'}</span>
                  </div>
                </div>
                <div className="flex gap-2">
          ...
          <div className="fixed inset-0 z-[100] flex flex-col justify-end bg-black/60 p-4">
            <motion.div initial={{ y: 300 }} animate={{ y: 0 }} exit={{ y: 300 }} className="bg-white p-6 rounded-3xl shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
              ...
              <form onSubmit={handleSaveBook} className="space-y-4">
                ...
                grid grid-cols-2 gap-4">
                   <div><label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-2 mb-1 block">Nivel MÃ­n</label><input type="number" value={bookForm.MínNivel} onChange={e=>setBookForm({...bookForm, MínNivel: Number(e.target.value)})} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl font-bold text-sm" /></div>
